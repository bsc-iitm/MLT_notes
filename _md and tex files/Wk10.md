\title{Week-10: Support Vector Machine}
...
Here, $\alpha$ is a vector of Lagrange multipliers, constrained to be non-negative ($\alpha \ge \mathbf{0}$).

Maximizing the Lagrange function with respect to $\alpha$ leads us to the following formulation:

\begin{align*}
\max_{\alpha \ge \mathbf{0}} \mathcal{L}(\mathbf{w}, \alpha) &= \max_{\alpha \ge \mathbf{0}} \left(f(\mathbf{w}) + \alpha^T g(\mathbf{w})\right) \\
&=
\begin{cases}
...
\begin{align*}
\min_{\mathbf{w}} f(\mathbf{w}) &= \min_{\mathbf{w}} \left[\max_{\alpha \ge \mathbf{0}} \mathcal{L}(\mathbf{w}, \alpha)\right] \\
&= \min_{\mathbf{w}} \left[\max_{\alpha \ge \mathbf{0}} \left(f(\mathbf{w}) + \alpha^T g(\mathbf{w})\right)\right]
\end{align*}
...
$$
\min_{\mathbf{w}} \left[\max_{\alpha \ge \mathbf{0}} \left(f(\mathbf{w}) + \alpha^T g(\mathbf{w})\right)\right] \equiv \max_{\alpha \ge \mathbf{0}} \left[\min_{\mathbf{w}} \left(f(\mathbf{w}) + \alpha^T g(\mathbf{w})\right)\right]
$$
...
\begin{align*}
\min_{\mathbf{w}} f(\mathbf{w}) &\equiv \min_{\mathbf{w}} \left[\max_{\alpha \ge \mathbf{0}} f(\mathbf{w}) + \sum _{i=1} ^m \alpha _i g_i(\mathbf{w})\right] \\
&\equiv \max_{\alpha \ge \mathbf{0}} \left[\min_{\mathbf{w}} f(\mathbf{w}) + \sum _{i=1} ^m \alpha _i g_i(\mathbf{w})\right]
\end{align*}
...
\begin{align*}
\mathcal{L}(\mathbf{w}, \alpha) &= \frac{1}{2}||\mathbf{w}||^2_2 + \sum _{i=1} ^n \alpha _i (1 - (\mathbf{w}^T\mathbf{x}_i)y_i)
\end{align*}

Now, considering the duality principle, we arrive at:

\begin{align*}
\min_{\mathbf{w}} \max_{\alpha \ge \mathbf{0}} \left[\frac{1}{2}||\mathbf{w}||^2_2 + \sum _{i=1} ^n \alpha _i (1 - (\mathbf{w}^T\mathbf{x}_i)y_i)\right] &\equiv \max_{\alpha \ge \mathbf{0}} \min_{\mathbf{w}} \left[\frac{1}{2}||\mathbf{w}||^2_2 + \sum _{i=1} ^n \alpha _i (1 - (\mathbf{w}^T\mathbf{x}_i)y_i)\right]
\end{align*}

Solving for the inner function of the dual problem, we obtain:

\begin{align*}
\mathbf{w}_{\alpha}^* - \sum _{i=1} ^n \alpha _i \mathbf{x}_i y_i = 0
\end{align*}

This leads to a vectorized form:

$$
\mathbf{w}_{\alpha}^* = \mathbf{XY}\alpha \quad \ldots[1]
$$
...
\begin{align*}
&\max_{\alpha \ge \mathbf{0}} \left[ \frac{1}{2}||\mathbf{w}||^2_2 + \sum _{i=1} ^n \alpha _i (1 - (\mathbf{w}^T\mathbf{x}_i)y_i) \right] \\
&= \max_{\alpha \ge \mathbf{0}} \left[ \frac{1}{2}\mathbf{w}^T\mathbf{w} + \sum _{i=1} ^n \alpha_i -  \mathbf{w}^T\mathbf{XY}\alpha \right] \\
&= \max_{\alpha \ge \mathbf{0}} \left[ \frac{1}{2}(\mathbf{XY}\alpha)^T(\mathbf{XY}\alpha) + \sum _{i=1} ^n \alpha_i -  (\mathbf{XY}\alpha)^T(\mathbf{XY}\alpha) \right] \quad \ldots\text{from }[1] \\
&= \max_{\alpha \ge \mathbf{0}} \left[ \sum _{i=1} ^n \alpha_i -  \frac{1}{2}(\mathbf{XY}\alpha)^T(\mathbf{XY}\alpha) \right] \\
\end{align*}

# Observations

1. **Dual and Primal Variables:** The duality of the SVM problem manifests in the dimensions of its variables. The dual problem is characterized by $\alpha \ge \mathbf{0}$, residing in $\mathbb{R}^n_+$, while the primal problem deals with $\mathbf{w}$ in $\mathbb{R}^d$.
...
$$
\min_{\mathbf{w}} \left [\max_{\alpha \ge \mathbf{0}} f(\mathbf{w}) + \alpha^T g(\mathbf{w}) \right ] \equiv \max_{\alpha \ge \mathbf{0}} \left [ \min_{\mathbf{w}} f(\mathbf{w}) + \alpha^T g(\mathbf{w}) \right ]
$$

Here, the primal function is on the left-hand side, and the dual function is on the right. The solutions for the primal and dual functions are denoted as $\mathbf{w}^∗$ and $\alpha^*$, respectively. When these solutions are substituted into the equation, we obtain:

$$
\max_{\alpha \ge \mathbf{0}} f(\mathbf{w}^*) + \alpha^T g(\mathbf{w}^*) =  \min_{\mathbf{w}} f(\mathbf{w}) + \alpha^{*T} g(\mathbf{w})
$$
...
$$
f(\mathbf{w}^*) \le f(\mathbf{w}^*) + \alpha^{*T} g(\mathbf{w}^*)
$$

Hence, we can infer:

$$
\alpha^{*T} g(\mathbf{w}^*) \ge \mathbf{0}
$$

However, considering the constraints, where $\alpha^{*} \ge \mathbf{0}$ and $g(\mathbf{w}^*)\le \mathbf{0}$, we arrive at:

$$
\alpha^{*T} g(\mathbf{w}^*) \le \mathbf{0}
$$

From this, we deduce:

$$
\alpha^{*T} g(\mathbf{w}^*) = \mathbf{0}
$$
...
2. **Gradient Descent:** Employ the gradient of the dual formula, $\alpha^T\mathbf{1} -  \frac{1}{2}\alpha^T\mathbf{Y}^T Q\mathbf{Y}\alpha$, in a gradient descent algorithm to iteratively find a satisfactory set of Lagrange multipliers $\alpha$. Initialize $\alpha$ as a zero vector in $\mathbb{R}^n_+$.

3. **Prediction:** For prediction, follow these formulas:
    * For non-kernelized SVM: $\text{label}(\mathbf{x}_{\text{test}}) = \text{sign}( \mathbf{w}^T\mathbf{x}_{\text{test}}) = \text{sign}\left(\sum _{i=1} ^n \alpha _i y_i(\mathbf{x}_i^T\mathbf{x}_{\text{test}})\right)$
    * For kernelized SVM: $\text{label}(\mathbf{x}_{\text{test}}) = \text{sign}(\mathbf{w}^T\boldsymbol{\phi}(\mathbf{x}_{\text{test}})) = \text{sign}\left(\sum _{i=1} ^n \alpha _i y_i k(\mathbf{x}_i^T\mathbf{x}_{\text{test}})\right)$
