...
3. To predict:
	* For non-kernelized SVM: $\text{label}(x_{test}) = \text{sign}( w^Tx_{test}) = \text{sign}\left(\sum _{i=1} ^n \alpha _i y_i(x_i^Tx_{test})\right)$
	* For kernelized SVM: $\text{label}(x_{test}) = \text{sign}(w^T\phi(x_{test})) = \text{sign}\left(\sum _{i=1} ^n \alpha _i y_ik(x_i^Tx_{test})\right)$
